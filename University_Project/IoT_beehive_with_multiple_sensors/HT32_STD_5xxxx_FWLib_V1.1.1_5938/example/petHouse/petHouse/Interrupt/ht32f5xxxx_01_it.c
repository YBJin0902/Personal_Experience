...
void HTCFG_UART_IRQHandler(void)
{
  ...
  if (USART_GetFlagStatus(HTCFG_UART_PORT, USART_FLAG_RXDR))
  {
    ...
    DBG_IO1_HI();
		...
		RXdata = USART_ReceiveData(HTCFG_UART_PORT);
...
void COM1_IRQHandler(void)
{
  ...
  if (USART_GetFlagStatus(COM1_PORT, USART_FLAG_RXDR))
  {
    ...
    //DBG_IO1_HI();
		...
		RXRD = USART_GetFlagStatus(COM1_PORT, USART_FLAG_RXDR);
		rx_data = USART_ReceiveData(COM1_PORT);
  ...
  /* Tx, move data from buffer to UART                                                                      */
  if (USART_GetIntStatus(COM1_PORT, USART_INT_TXDE) &&
      USART_GetFlagStatus(COM1_PORT, USART_FLAG_TXDE))
  {
    DBG_IO2_LO();
    if (gURTx_Length > 0)
    {
      USART_SendData(COM1_PORT, *gURTx_Ptr++);
      gURTx_Length--;
      if (gURTx_Length == 0)
      {
  ...
  if (USART_GetIntStatus(COM1_PORT, USART_INT_TXC) &&
      USART_GetFlagStatus(COM1_PORT, USART_FLAG_TXC))
  {
    USART_IntConfig(COM1_PORT, USART_INT_TXC, DISABLE);
    gIsTxFinished = TRUE;
  }
}
...
void COM2_IRQHandler(void)
{
  ...
  if (USART_GetFlagStatus(COM2_PORT, USART_FLAG_RXDR))
  {
    ...
    //DBG_IO1_HI();
		...
		LoRadata = USART_ReceiveData(COM2_PORT);
		LoRa_BUF[LoRa_Index++] = LoRadata;
  ...
  /* Tx, move data from buffer to UART                                                                      */
  if (USART_GetIntStatus(COM2_PORT, USART_INT_TXDE) &&
      USART_GetFlagStatus(COM2_PORT, USART_FLAG_TXDE))
  {
    DBG_IO2_LO();
    if (gURTx_Length > 0)
    {
      USART_SendData(COM2_PORT, *gURTx_Ptr++);
      gURTx_Length--;
      if (gURTx_Length == 0)
      {
  ...
  if (USART_GetIntStatus(COM2_PORT, USART_INT_TXC) &&
      USART_GetFlagStatus(COM2_PORT, USART_FLAG_TXC))
  {
    USART_IntConfig(COM2_PORT, USART_INT_TXC, DISABLE);
    gIsTxFinished = TRUE;
  }
}
...
void COM3_IRQHandler(void)
{
  ...
  if (USART_GetFlagStatus(COM3_PORT, USART_FLAG_RXDR))
  {
    ...
    //DBG_IO1_HI();
		...
		SOLARdata = USART_ReceiveData(COM3_PORT);
		SOLAR_BUF[SOLAR_Index++] = SOLARdata;
  ...
  /* Tx, move data from buffer to UART                                                                      */
  if (USART_GetIntStatus(COM3_PORT, USART_INT_TXDE) &&
      USART_GetFlagStatus(COM3_PORT, USART_FLAG_TXDE))
  {
    DBG_IO2_LO();
    if (gURTx_Length > 0)
    {
      USART_SendData(COM3_PORT, *gURTx_Ptr++);
      gURTx_Length--;
      if (gURTx_Length == 0)
      {
        USART_IntConfig(COM3_PORT, USART_INT_TXDE, DISABLE);
      }
    }
    DBG_IO2_HI();
  }

  if (USART_GetIntStatus(COM3_PORT, USART_INT_TXC) &&
      USART_GetFlagStatus(COM3_PORT, USART_FLAG_TXC))
  {
    USART_IntConfig(COM3_PORT, USART_INT_TXC, DISABLE);
    gIsTxFinished = TRUE;
  }
}
