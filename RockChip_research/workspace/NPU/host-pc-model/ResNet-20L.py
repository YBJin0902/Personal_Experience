...
def main():
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(os.path.join(args.out_dir, "resnet-20l-best.keras"),
    ...
    with open(os.path.join(args.out_dir, "resnet-20l-labels.json"), "w", encoding="utf-8") as f:
        json.dump(LABELS_10, f, indent=2, ensure_ascii=False)

    with open(os.path.join(args.out_dir, "resnet-20l-history.json"), "w", encoding="utf-8") as f:
        json.dump(hist.history, f, indent=2, ensure_ascii=False)

    model.save(os.path.join(args.out_dir, "resnet-20l-final.keras"))
    print(f"\n[Saved] {args.out_dir}/resnet-20l-best.keras, resnet-20l-final.keras, resnet-20l-labels.json")
