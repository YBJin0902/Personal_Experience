...
LABELS = ["black","blue","cyan","green","orange","pink","purple","red","white","yellow"]
REQ_COLS = ["X","Y","Lv","x","y"]
...
def read_one_row_xy(csv_path: str) -> np.ndarray:
    ...
    x = float(df.loc[df.index[0], "x"])
    y = float(df.loc[df.index[0], "y"])

    return np.array([
        df.loc[df.index[0], "X"],
        df.loc[df.index[0], "Y"],
        df.loc[df.index[0], "Lv"],
        df.loc[df.index[0], "x"],
        df.loc[df.index[0], "y"],
    ], dtype=np.float32)
...
def build_model(n_classes=10, hidden=64, dropout=0.2):
    inp = keras.Input(shape=(5,), name="xy")
    ...
    x = keras.layers.Dense(hidden, activation="relu", name="fc3")(x)
    x = keras.layers.Dropout(dropout, name="drop3")(x)
...
def main():
    ...
    ap.add_argument("--data_root", type=str, default="dataset/train/",
                    help="dataset/train/<label>/*.csv（每檔一筆，含X Y Z x y）")
    ...
    model = build_model(n_classes=len(LABELS), hidden=args.hidden, dropout=args.dropout)
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(os.path.join(args.out_dir, "mlp-best.keras"),
    ...
    with open(os.path.join(args.out_dir, "mlp-labels.json"), "w", encoding="utf-8") as f:
        json.dump(LABELS, f, indent=2, ensure_ascii=False)
    with open(os.path.join(args.out_dir, "mlp-history.json"), "w", encoding="utf-8") as f:
        json.dump(hist.history, f, indent=2, ensure_ascii=False)

    model.save(os.path.join(args.out_dir, "mlp-final.keras"))
    print(f"\n[Saved] {args.out_dir}/mlp-best.keras, mlp-final.keras, mlp-labels.json")
